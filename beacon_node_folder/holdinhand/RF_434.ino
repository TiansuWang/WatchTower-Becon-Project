...
void validate_packet_434(uint8_t * receive_buffer,uint8_t len){
    ...
    String config_command = receive_string.substring(0,3);
    if(packet_header.compareTo("WTRCO") == 0){
      Serial.println("robot passed!!!!!");
      if(!robot_passed_flag){
        rbt_pass_hour_start = greenwich_hour;
        rbt_pass_minute_start = greenwich_minute;
        rbt_pass_seconds_start = greenwich_seconds;
      }
      beacon_Rssi = Rssi;
      rbt_pass_hour_end = greenwich_hour;
      rbt_pass_minute_end = greenwich_minute;
      rbt_pass_seconds_end = greenwich_seconds;
      if(Rssi > MaxRssi){
        rbt_pass_hour_MaxRssi = greenwich_hour;
        rbt_pass_minute_MaxRssi = greenwich_minute;
        rbt_pass_seconds_MaxRssi = greenwich_seconds;
        MaxRssi = Rssi;
      }
      ...
      //demonstrat data for test
      Serial.print("max Rssi time:");
      Serial.print(rbt_pass_hour_MaxRssi); Serial.print(':');
      Serial.print(rbt_pass_minute_MaxRssi); Serial.print(':');
      Serial.println(rbt_pass_seconds_MaxRssi); 
      
      Serial.print("start time:");
      Serial.print(rbt_pass_hour_start); Serial.print(':');
      Serial.print(rbt_pass_minute_start); Serial.print(':');
      Serial.println(rbt_pass_seconds_start); 

      Serial.print("end time:");
      Serial.print(rbt_pass_hour_end); Serial.print(':');
      Serial.print(rbt_pass_minute_end); Serial.print(':');
      Serial.println(rbt_pass_seconds_end); 
      
    }else if(config_command.compareTo("CON") == 0 & (!config_done_flag)){
      Serial.println("get configure command:");
      Serial.print("ID:");Serial.print(*(receive_buffer+3));
      Serial.print("DEST:");Serial.print(*(receive_buffer+4));
      node_ID = *(receive_buffer+3);
      unicast_destination = *(receive_buffer+4);
      delay(100);
       uint8_t reply[5] = {};
      // RPL 
      reply[0] = 0x52;
      ...
      reply[2] = 0x4C;
      reply[3] = node_ID;
      reply[4] = unicast_destination;
      transmit_434(reply,5);
      update_OLED();
      
    }else if(config_command.compareTo("ECH") == 0 & (!config_done_flag)){
      uint8_t echo_message[5] = {};
      echo_message[0] = unicast_destination;
      echo_message[1] = node_ID;
      echo_message[2] = 0x45;
      echo_message[3] = 0x43;
      echo_message[4] = 0x48;
      transmit(echo_message,5);
      lasttimeSendEcho = millis();
      echo_timeout_on = true;
      receive_915(0x00,0x00,0x00);
    }else if(config_command.compareTo("LEC") == 0 & (!config_done_flag)){
      Serial.println("LEC");
      long_echo_dest = *(receive_buffer+3);
      long_echo_source = node_ID;
      initiate_long_echo(long_echo_dest);
      
    }else if(config_command.compareTo("RCN") == 0){
      Serial.println(*(receive_buffer+3));
      if(*(receive_buffer+3) == node_ID){
        config_done_flag = false;
        delay(100);
        uint8_t RCB[5] = {};
        // RCB 0x52 0x43 0x42 
        RCB[0] = 0x52;
    ...
    }else if(config_command.compareTo("DON") == 0){
        config_done_flag = true;
        update_OLED();
 ...
 void initiate_long_echo(uint8_t long_echo_dest){
  Serial.println("goes in initiate_long_echo");
  if(long_echo_dest > node_ID){
        if(unicast_backward_destination == 255){
          Serial.print("backward_destination not assigned");
      ...
      }else if(long_echo_dest < node_ID){
        long_echo_message[0] = unicast_destination;
      }else{
        //send back own LEB report.
        generate_LEB_message();
        uint8_t self_generate_LEB_data_len = LEB_message_length-5;
        uint8_t self_generate_LEB_data[self_generate_LEB_data_len];
        for(int i = 0;i < LEB_message_length-5; i++){
      ...
      long_echo_message[5] = long_echo_dest;
      long_echo_message[6] = node_ID;
      long_echo_message[7] = 0;
      transmit(long_echo_message,8);
...
void RF_reset_434(){
  ...
  for (int i=0;i<3;i++){
    ...
    //Serial.print(*(ptr));Serial.println(*(ptr+1));
    if (*ptr == 2 & *(ptr+1) ==1 ){
  ...
  Serial.println("434 Reset unsuccess.");
  while (1);
}
...
void clear_IRQ_434(){
    ...
    uint8_t ClearIrqCount = 0;
      if (*(ptr3+1) ==0){
        ...
        last_434_error = "434 reset IRQ error.";
        update_OLED();
        do{
          wait_434();
          ClearIrqStatus_434();  
